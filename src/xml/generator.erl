...
% main function
generate(MessageSchema, TypeMap, MessageMap) -> 
    lists:map(
        fun(ID) -> generateMessageStub(MessageSchema, TypeMap, dict:fetch(ID, MessageMap)) end,
        dict:fetch_keys(MessageMap)).


% generate message.erl for single message
generateMessageStub(MessageSchema, TypeMap, MessageNode) -> 
    ...
    % TODO: pass out_dir as command-line argument
    DirPath = "../example/",
    ModuleName = utils:lowerfirst(dict:fetch(name, MessageAttributes)),
    FilePath = filename:join([DirPath, ModuleName ++ ".erl"]),
